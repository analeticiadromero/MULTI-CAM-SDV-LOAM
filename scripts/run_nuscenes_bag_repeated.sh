...
RUNS=15
TAG="nuscenes_scene0061_6cam"
BAG_DIR="/media/analeticiaromero/ExtremeSSD/ANA/nuscenes_bags"
BAG_FILE="61.bag"
ROS_DISTRO_NAME="${ROS_DISTRO:-melodic}"
SDV_WS="$ROOT_DIR"
SDV_LAUNCH="run_nuscenes_6cam.launch"
OUTPUT_DIR="$ROOT_DIR/src/sdv_loam/output"
ARCHIVE_DIR=""
SDV_WARMUP_SEC=5
POST_BAG_WAIT_SEC=5
BAG_DELAY_SEC=1.0
...
Example:
  $0 --runs 15 --tag scene0061_6cam --bag-dir /media/analeticiaromero/ExtremeSSD/ANA/nuscenes_bags --bag-file 61.bag
EOF
}
...
start_sdv() {
    ...
    echo "Starting SDV-LOAM multicam for run ${run_id}"
    mkdir -p "$OUTPUT_DIR"
...
start_bag() {
    local run_id="$1"
    local log_file="$ARCHIVE_DIR/bag_${TAG}_run$(printf '%02d' "$run_id").log"
    echo "Starting rosbag play for run ${run_id}"
...
for run_id in $(seq 1 "$RUNS"); do
    echo
    echo "========== nuScenes 6cam bag run ${run_id}/${RUNS} =========="
    SDV_PID=""
    BAG_PID=""
    ...
    if [[ "$bag_status" -eq 124 ]]; then
        echo "rosbag play reached timeout (${TIMEOUT_SEC}s) in run ${run_id}; stopping bag." >&2
        cleanup_process_group "$BAG_PID" "rosbag play"
    elif [[ "$bag_status" -ne 0 ]]; then
        echo "rosbag play exited with status ${bag_status} in run ${run_id}." >&2
    fi
    BAG_PID=""
